...
pub enum MinecraftAuthorizationError<E: std::error::Error> {
    ...
    /// The server responded with a non-success status code at a specific step of the flow.
    /// `step` identifies which request failed: "xbl", "xsts" or "minecraft".
    #[error("HTTP status error at {step}: {status}")]
    HttpStatusAtStep { step: &'static str, status: StatusCode },
...
impl<C: HttpClient> MinecraftAuthorizationFlow<C> {
    ...
    pub async fn exchange_microsoft_token(
        ...
        if !response.status().is_success() {
            return Err(MinecraftAuthorizationError::HttpStatusAtStep {
                step: "minecraft",
    ...
    async fn xbox_security_token(
        ...
        if response.status() == StatusCode::UNAUTHORIZED {
            ...
            Err(MinecraftAuthorizationError::HttpStatusAtStep {
                step: "xsts",
                status: response.status(),
            })
        } else {
