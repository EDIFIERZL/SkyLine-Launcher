...
import { togglePlay, next, prev, seekTo, setVolume } from './musicEngine'
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 px-2.5 h-9 select-none">
          ...
          <ChevronDown className={`w-3 h-3 text-surface-400 shrink-0 transition-transform duration-300 ${hover ? 'rotate-180' : ''}`} />
        </div>

        <div
          className="overflow-hidden"
          style={{ maxHeight: hover ? 260 : 0, transition: `max-height 0.4s ${spring}` }}
        >
          <div
